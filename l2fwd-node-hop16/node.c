...
using namespace chrono; //time-related

// struct rte_hash* id_mac_hash_table;
...
#define FAIL_STORE -5 
#define SUCCESS_STORE 5 

#define RTE_LOGTYPE_L2FWD RTE_LOGTYPE_USER1

// #define NB_MBUF   8192  //new original node code setting
#define NB_MBUF   65536 //new source node code setting
...
/*Two structs, rte_eth_rxconf and rte_eth_txconf, are defined to configure the behavior of the receive (RX) and send (TX) queues, respectively*/
...
/* Print out statistics of rte_mmbuf */
...
static int append_data(struct rte_mbuf *m, const void *data, size_t len) {
    ...
    rte_memcpy(tail, data, len);// Copy data to the tail of the mbuf
...
/*Extracts (tailing) size bytes of data from the rte_mbuf data structure and returns a copy of those bytes*/
static unsigned char* get_bytes_fromRightmost(struct rte_mbuf* m, int size, bool if_clear, data_type_t data_type)
{
    
    int data_length = m->data_len;

    if(size > data_length ){
        return NULL;
    }
    unsigned char * data = (unsigned char*)m->buf_addr + m->data_off + data_length - size;
	void *ret = NULL;

    switch (data_type) {
        case UINT8:
            ret = malloc(sizeof(uint8_t));  // Allocate space for uint8_t

            if (ret) {
                *(uint8_t*)ret = *data;  // Extract uint8 t data
            }
            break;
        case UINT32:
            ret = malloc(sizeof(uint32_t));  // Allocate space for uint32_t
            if (ret) {
                *(uint32_t*)ret = *(uint32_t*)data;  // Extract uint32 t data
            }
            break;
        case UINT64:
            ret = malloc(sizeof(uint64_t));  // Allocate space for uint64_t
            if (ret) {
                *(uint64_t*)ret = *(uint64_t*)data;  // Extract uint64 t data
            }
            break;
        case UINT8_ARRAY:
            ret = malloc(size);  // Allocate space for uint8_t arrays
            if (ret) {
                for (int i = 0; i < size; i++) {
                    ((unsigned char*)ret)[i] = data[i];  // Extract uint8_t arrays
                }
            }
            break;
        default:
            return NULL;  // Unsupported data types

    }

	if( if_clear ){ /* if we need to trim some part of the mbuf*/
		rte_pktmbuf_trim(m,size);
	}
    return (unsigned char*)ret;
}

/*Extracts the size bytes of data from the rte_mbuf data structure with a tail offset offset and returns a copy of those bytes*/
static unsigned char* get_bytes_fromRightmost_offset(struct rte_mbuf* m, 
                                                     int size, 
                                                     int offset, 
                                                     data_type_t data_type)
{
    /* The current MBUF effective data length */
    int data_length = m->data_len;

    /* check whether offset + size exceeds the actual data length */
    if (offset + size > data_length) {
        return NULL;
    }

    /*
     *   Calculate the starting position to read:
     *   Move offset forward from data_length (end) and then size forward
     *   Finally, you get the starting address to copy the data
     */
    unsigned char* data = (unsigned char*)m->buf_addr
                          + m->data_off
                          + data_length
                          - offset
                          - size;
    ...
    /* Allocate space and copy data depending on data_type */
    switch (data_type) {
        case UINT8:
        ...
        default:
            /* Unsupported data types, which return NULL directly */
            return NULL;
    }
...
/*Parses the contents of the DataPackage object from the rte_mbuf packet*/
static int Get_DataPackage(struct rte_mbuf* m, DataPackage * datapackage)
{
	unsigned char* temp;
	for(int i = N-1; i >=0; i--){
		temp = get_bytes_fromRightmost(m, 4, true, UINT32);
		memcpy(&(datapackage -> PVhd.POT[i]), temp, 4);
		// printf("get in -> PVhd.POT[ %d ]: ", i);
        // print_binary_32(datapackage -> PVhd.POT[i]);
	}

	temp = get_bytes_fromRightmost_offset(m, 1, 0, UINT8);
	memcpy(&(datapackage -> PVhd.flag), temp, 1);

	temp = get_bytes_fromRightmost_offset(m, 16, 1, UINT8_ARRAY);
	memcpy(datapackage -> PVhd.pkt_ID, temp, 16);

	temp = get_bytes_fromRightmost_offset(m, 4, 17, UINT32);
	memcpy(&(datapackage -> PVhd.timestamp), temp, 4);

	temp = get_bytes_fromRightmost_offset(m, 1, 21, UINT8);
	memcpy(&(datapackage -> PVhd.pot_length), temp, 1);

	temp = get_bytes_fromRightmost_offset(m, 1, 22, UINT8);
	memcpy(&(datapackage -> PVhd.pkt_seq), temp, 1);
	// printf("get pktID pkt_seq = %d\n", (datapackage -> PVhd.pkt_seq));


	temp = get_bytes_fromRightmost_offset(m, 1, 23, UINT8);
	memcpy(&(datapackage -> PVhd.group_size), temp, 1);

	temp = get_bytes_fromRightmost_offset(m, 8, 24, UINT64);
	memcpy(&(datapackage -> PVhd.group_ID), temp, 8);
	// printf("get pktID group_ID = %d\n", (datapackage -> PVhd.group_ID));

	temp = get_bytes_fromRightmost_offset(m, 1, 32, UINT8);
	memcpy(&(datapackage -> PVhd.hd_length), temp, 1);
...
static int package_process(struct rte_mbuf* m, int portid)
{
		...
		DataPackage * mid_datapackage = new DataPackage();
		/* 1 get the package */
		Get_DataPackage(m,mid_datapackage);

		tick_ve1 = rdtsc();
		signers[portid] -> generateUpdate(mid_datapackage, portid);
		tick_ve2 = rdtsc();	
		time_ve = (double)(tick_ve2 - tick_ve1) / 2.4;

		
		/* 4 store */
		Store_DataPackage(m,mid_datapackage);
...
l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid)
{
    ...
    uint32_t new_src_ip = rte_cpu_to_be_32(0xAC113C9D);  // 172.17.60.157 new node6 eth1 0xAC113C9D
    uint32_t new_dst_ip = rte_cpu_to_be_32(0xAC113C9F);  // 172.17.60.159 new node7 eth1 0xAC113C9F
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			nb_rx = rte_eth_rx_burst((uint8_t) portid, 0,
						 pkts_burst, MAX_PKT_BURST);// Receive the 0 rx queue of the port
			

			port_statistics[portid].rx += nb_rx;	
            /*Traverse the received data packets.*/
			for (j = 0; j < nb_rx; j++) {
				m = pkts_burst[j]; //pkts_burst[j] is the rte_mbuf pointer to the jth packet received
				...
				if(ipv4_hdr->src_addr == new_src_ip){
					...
					// unsigned long long tick_forward0;
					// unsigned long long tick_forward1;
					// tick_forward0 = rdtsc();
					l2fwd_simple_forward(m, portid);
					// tick_forward1 = rdtsc();
					
					// tick2 =  rdtsc();
					// time_throughput = (double)(tick2 - tick0) / 2.4;
					// packet_number[portid]--;
					// printf("packetnumber = %d\ntime = %f ns\npacket size = %d\nthroughput = %f Gbps\n", packet_number[portid], time_throughput , (datalength * G + 19 * G + 16 * N + 52), (datalength * G + 19 * G + 16 * N + 52) * 8 / time_throughput);
					// printf("verify time = %f ns\n", time_ve / G);
					// double time_forward = (double)(tick_forward1 - tick_forward0) / 2.4;
					// printf("forward time = %f ns\n", time_forward);
					// printf("R/S time = %f ns\n\n", time_throughput - time_ve);
				}			
...
main(int argc, char **argv)
{
	...
	// hash_params.name = "id_mac_hash_table";
	// hash_params.entries = MAX_ENTRIES;
	// hash_params.key_len = sizeof(uint64_t);
	// hash_params.hash_func = NULL; 
	// hash_params.hash_func_init_val = 0;
	
	// id_mac_hash_table = rte_hash_create(&hash_params);
    // if (id_mac_hash_table == NULL) {
